...
* `--create-batch-size`: create flush size. With `--transport pipeline`, this is native pipeline depth for singular `FLOW.CREATE`. With `--transport many`, this is `FLOW.CREATE_MANY` size.
...
* `--transport pipeline`: use normal SDK calls over a buffered command executor.
  Create uses singular `FLOW.CREATE` in native pipeline flushes. Workers call
  `FLOW.CLAIM_DUE` immediately, then flush singular `FLOW.COMPLETE` commands in
  a native pipeline. This is the optimized developer-facing path.
...
used for memtier-style testing: same data dir, shard count, Erlang flags,
